...
unsafe extern "C" fn buddy_special_s_dash_pre(fighter: &mut L2CFighterCommon) -> L2CValue {
...
unsafe extern "C" fn buddy_special_s_dash_main(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    fighter.sub_change_motion_by_situation(
        hash40("special_s_dash").into(),
        hash40("special_air_s_dash").into(),
        false.into()
    );

    WorkModule::count_down_int(fighter.module_accessor, *FIGHTER_BUDDY_INSTANCE_WORK_ID_INT_SPECIAL_S_REMAIN, 0);

    fighter.sub_shift_status_main(L2CValue::Ptr(buddy_special_s_dash_main_loop as *const () as _))
}
...
unsafe extern "C" fn buddy_special_s_dash_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if fighter.global_table[SITUATION_KIND].get_i32() == *SITUATION_KIND_GROUND {
        ...
        if degree_min <= degree {
            // I assume this bonks if the angle of the ground is too high
        ...
        if degree_min <= degree_prev {
            // I assume this lets Banjo run off ledge if the angle is too steep
...
unsafe extern "C" fn buddy_special_s_dash_kinetic_helper(fighter: &mut L2CFighterCommon, inherit_speed: L2CValue) {
    ...
    if fighter.global_table[SITUATION_KIND].get_i32() != *SITUATION_KIND_GROUND {
        ...
        sv_kinetic_energy!(
            ...
            fighter,
            FIGHTER_KINETIC_ENERGY_ID_GRAVITY,
            stable_speed_y * dash_gravity_limit_mul
        );
...
unsafe extern "C" fn buddy_special_s_dash_end(fighter: &mut L2CFighterCommon) -> L2CValue {
    buddy_special_s_remove_armor(fighter);
    0.into()
}

pub fn install(agent: &mut smashline::Agent) {
    agent.status(Pre, *FIGHTER_BUDDY_STATUS_KIND_SPECIAL_S_DASH, buddy_special_s_dash_pre);
    agent.status(Main, *FIGHTER_BUDDY_STATUS_KIND_SPECIAL_S_DASH, buddy_special_s_dash_main);
    agent.status(End, *FIGHTER_BUDDY_STATUS_KIND_SPECIAL_S_DASH, buddy_special_s_dash_end);
}